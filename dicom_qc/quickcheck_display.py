...
class QuickCheckDisplayMixin:
    ...
    def _get_thumbnail_for_display(self, series) -> Optional[str]:
        ...
        if (
            series._thumbnail_path
            and hasattr(self, "_thumb_cache")
            and self._thumb_cache
        ):
    ...
    def display(self, use_pagination: bool = None):
        ...
        subject_options = [("All Subjects", None)] + [
            (p.label, pid) for pid, p in sorted(self.patients.items())
        ]
        session_options = [("All Sessions", None)]

        # Build series number options (sorted numerically)
        series_numbers = sorted(
            set(s.series_number for s in all_series if s.series_number is not None),
            key=self._series_num_sort_key,
        )
        ...
        summary_parts = [
            f'<span style="font-weight:600;font-size:14px;">{len(all_series)}</span> series'
        ]
        for status in ["PASS", "FAIL", "WARNING", "ERROR", "NOTE", "DERIVED"]:
            if counts.get(status, 0) > 0:
                text_color = "#333" if status == "WARNING" else "#fff"
                summary_parts.append(
                    f'<span style="background:{c[status]};color:{text_color};padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600;">{counts[status]} {status}</span>'
                )
        summary_html = " ".join(summary_parts)

        # === Filter widgets (compact) ===
        status_dropdown = widgets.Dropdown(
            options=status_options, value="ALL", layout=widgets.Layout(width="120px")
        )
        subject_dropdown = widgets.Dropdown(
            options=subject_options, value=None, layout=widgets.Layout(width="160px")
        )
        session_dropdown = widgets.Dropdown(
            options=session_options, value=None, layout=widgets.Layout(width="180px")
        )
        series_num_dropdown = widgets.Dropdown(
            options=series_num_options, value=None, layout=widgets.Layout(width="70px")
        )
        description_filter = widgets.Text(
            placeholder="Filter...", layout=widgets.Layout(width="120px")
        )

        # === Left panel: scrollable list with buttons ===
        list_area = widgets.Output(
            layout=widgets.Layout(
                height="80vh", width="98%", overflow_y="auto", background="#fafafa"
            )
        )
        list_area.add_class("qc-list-area")

        # === Right panel: single content area with toggle ===
        content_panel = widgets.Output(
            layout=widgets.Layout(
                min_height="80vh",
                overflow="auto",
        ...
        toggle_container = widgets.VBox(
            [content_panel], layout=widgets.Layout(width="100%", min_height="80vh")
        )
        ...
        def clear_content():
            ...
            if loaded_data["header_viewer"] is not None:
                loaded_data["header_settings"] = loaded_data[
                    "header_viewer"
                ].get_settings()
        ...
        def render_header_view():
            ...
            with content_panel:
                ...
                if dicom_files:
                    ...
                    display(
                        widgets.HTML(
                            '<div style="color:#64748b;padding:30px;">No DICOM files available for this series</div>'
                        )
                    )
            ...
            for patient_id, patient in sorted(self.patients.items()):
                for study_key, study in sorted(
                    ...
                    for series in sorted(
                        study.series.values(),
                        key=lambda s: self._series_num_sort_key(s.series_number),
                    ):
        ...
        def make_header_bar(series, patient, study):
            ...
            nav_buttons = widgets.HBox(
                [prev_btn, next_btn], layout=widgets.Layout(gap="4px")
            )
        ...
        def make_issues_banner(series):
            ...
            if not (
                series.qc_report and series.qc_status in ("FAIL", "WARNING", "NOTE")
            ):
                return None
            issues = [
                r
            ...
            issue_items = " · ".join(
                f"<b>{r.check_name}:</b> {r.message}" for r in issues
            )
            return widgets.HTML(f"""
            ...
            with content_panel:
                ...
                if volume is None:
                    ...
                    load_btn = widgets.Button(
                        description="Load Images", button_style="primary", icon="image"
                    )
                ...
                viewer = MultiViewViewer(
                    volume, dicom_files=dicom_files, show_title=False
                )
        ...
        def update_session_options(_=None):
            ...
            if subj_id is None:
                ...
                if patient:
                    opts = [("All Sessions", None)] + [
                        (s.label, sk) for sk, s in sorted(patient.studies.items())
                    ]
        ...
        def matches_filter(series, patient_id, study_key):
            ...
            if (
                ...
                and series.qc_status != status_dropdown.value
            ):
                return False
            # Subject filter
            if (
                subject_dropdown.value is not None
                and patient_id != subject_dropdown.value
            ):
                return False
            # Session filter
            if (
                session_dropdown.value is not None
                and study_key != session_dropdown.value
            ):
                return False
            # Series number filter
            if (
                series_num_dropdown.value is not None
                and series.series_number != series_num_dropdown.value
            ):
                return False
            # Description filter
                ...
                for patient_id, patient in sorted(self.patients.items()):
                    ...
                    for study_key, study in sorted(
                        ...
                        study_series = [
                            s
                            for s in study.series.values()
                            if matches_filter(s, patient_id, study_key)
                        ]
                        if not study_series:
                            continue
                        ...
                        for series in sorted(
                            ...
                            style = STATUS_STYLES.get(
                                series.qc_status, STATUS_STYLES["PENDING"]
                            )
                            card_bg = style["bg"]
                            ...
                            has_issues = (
                                series.qc_status in ("FAIL", "WARNING", "NOTE")
                                and series.qc_report
                            )
                            ...
                            if has_issues:
                                ...
                                if issues:
                                    issue_list = ", ".join(
                                        escape(r.check_name) for r in issues
                                    )
                            ...
                            badge_text = (
                                "#fff" if series.qc_status != "WARNING" else "#78350f"
                            )
                            ...
                            view_btn = widgets.Button(
                                ...
                                layout=widgets.Layout(
                                    flex="1", height="32px", min_width="0"
                                ),
                            )
                            ...
                            button_row = widgets.HBox(
                                [view_btn, tags_btn],
                                layout=widgets.Layout(
                                    margin="10px 0 0 0", gap="8px", width="100%"
                                ),
                            )
                            ...
                            card = widgets.VBox(
                                ...
                                layout=widgets.Layout(
                                    ...
                                    padding="12px",
                                    margin="0 0 10px 0",
                                    box_shadow=f"0 0 0 3px {border_color}"
                                    if is_selected
                                    else None,
                                ),
                            )
        ...
        def make_filter(label, widget):
            lbl = widgets.HTML(
                f'<span style="font-size:11px;color:#64748b;margin-right:4px;">{label}</span>'
            )
            return widgets.HBox(
                [lbl, widget], layout=widgets.Layout(align_items="center")
            )
        ...
        main_layout = widgets.HBox(
            [left_panel, right_panel], layout=widgets.Layout(width="100%")
        )

        display(
            widgets.VBox([header, main_layout], layout=widgets.Layout(width="100%"))
        )
    ...
    def _display_paginated(self):
            ...
            for study_key, study in patient.studies.items():
                for series_key, series in study.series.items():
                    effective_study_uid, effective_series_uid = (
                        self._get_effective_uids(series, study, study_key)
                    )
                    series_lookup[
                        (patient_id, effective_study_uid, effective_series_uid)
                    ] = (series, patient, study)
        ...
        # === Summary badges ===
        summary_parts = [
            f'<span style="font-weight:600;font-size:14px;">{total_series}</span> series'
        ]
        for status in ["PASS", "FAIL", "WARNING", "ERROR", "NOTE", "DERIVED"]:
            count = counts.get(status, 0)
            if count > 0:
        ...
        # === Filter widgets ===
        status_dropdown = widgets.Dropdown(
            options=status_options, value=None, layout=widgets.Layout(width="130px")
        )
        subject_dropdown = widgets.Dropdown(
            options=patient_options, value=None, layout=widgets.Layout(width="180px")
        )
        session_dropdown = widgets.Dropdown(
        ...
        series_num_dropdown = widgets.Dropdown(
            options=series_num_options, value=None, layout=widgets.Layout(width="80px")
        )
        description_filter = widgets.Text(
            placeholder="Filter description...", layout=widgets.Layout(width="150px")
        )

        # === Pagination controls ===
        prev_page_btn = widgets.Button(
            description="< Prev", disabled=True, layout=widgets.Layout(width="80px")
        )
        ...
        def render_page():
            ...
            series_rows = self._db.get_filtered_series(
        ...
        def render_series_card(row):
            ...
            if row["thumbnail_path"] and self._thumb_cache:
                thumbnail_b64 = self._thumb_cache.get_thumbnail_base64(
                    row["thumbnail_path"]
                )
            ...
            session_label = (
                ...
                or row["study_date"]
                or (row["study_uid"][:20] if row["study_uid"] else "")
            )
        ...
        # Start with viewer closed (thumbnails full width)
        left_panel = widgets.VBox(
            [list_area], layout=widgets.Layout(width="100%", flex="1 1 auto")
        )
        right_panel = widgets.VBox(
            [placeholder, toggle_container],
            layout=widgets.Layout(flex="1 1 auto", min_width="400px", display="none"),
        ...
        for patient_id, patient in sorted(self.patients.items()):
            ...
            for study_uid, study in sorted(
                ...
                for series_uid, series in sorted(
                    study.series.items(),
                    key=lambda item: self._series_num_sort_key(item[1].series_number),
                ):
