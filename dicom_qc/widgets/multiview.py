...
class MultiViewViewer(VolumeRenderer):
    ...
    def display(self, on_ready: Optional[callable] = None):
        ...
        interact_hint = (
            "Click to move crosshairs, scroll to change slice"
            if self._interactive
            else "Use sliders to navigate"
        )
        if self._show_title:
        ...
        slice_controls = widgets.HBox(
            ...
            layout=widgets.Layout(
                padding="8px 16px", gap="8px", align_items="center", flex_wrap="wrap"
            ),
        )
        ...
        controls = widgets.VBox(
            controls_children, layout=widgets.Layout(background="#ffffff")
        )
        ...
        if self._interactive:
            ...
            with self.figure_output:
                self._fig, self._axes = plt.subplots(
                    1, 3, figsize=(8, 3), facecolor="black"
                )
    ...
    def _render_figure(self):
        ...
        if self._interactive and self._fig is not None:
            ...
            with self.figure_output:
                ...
                display(
                    HTML(
                        f'<img src="data:image/jpeg;base64,{b64_data}" style="display:block;">'
                    )
                )
...
class DicomHeaderViewer:
    ...
    def _create_widgets(self):
        ...
        file_options = (
            [(f"File {i + 1} of {num_files}", i) for i in range(num_files)]
            if num_files > 0
            else [("No files", 0)]
        )
        ...
        self.quick_all_btn = widgets.Button(
            description="All",
            button_style="primary",
            layout=widgets.Layout(width="50px"),
        )
        self.quick_all_btn.on_click(lambda _: self._set_quick_filter("all"))
        self.quick_private_btn = widgets.Button(
            description="Private", layout=widgets.Layout(width="70px")
        )
        self.quick_private_btn.on_click(lambda _: self._set_quick_filter("private"))
        self.quick_seq_btn = widgets.Button(
            description="Seq", layout=widgets.Layout(width="50px")
        )
        ...
        for group_name in self.TAG_GROUPS.keys():
            btn = widgets.Button(
                description=group_name, layout=widgets.Layout(width="auto")
            )
    ...
    def _on_tag_search_change(self, change):
        ...
        for tag_str, keyword, display_name in self._all_tags:
            if (
                search in tag_str.lower()
                or search in keyword.lower()
    ...
    def _update_selected_tags_display(self):
        ...
        self.quick_all_btn.button_style = (
            "primary" if self._quick_filter == "all" else ""
        )
        self.quick_private_btn.button_style = (
            "primary" if self._quick_filter == "private" else ""
        )
        self.quick_seq_btn.button_style = (
            "primary" if self._quick_filter == "sequences" else ""
        )

        if self._quick_filter == "all":
            self.selected_tags_label.value = (
                '<span style="color:#888;font-size:11px;">Showing all tags</span>'
            )
            ...
            self.selected_tags_label.value = (
                '<span style="font-size:11px;color:#666;">Selected:</span>'
            )
            ...
            for tag_str, keyword in self._selected_tags:
                ...
                chip.on_click(
                    lambda _, ts=tag_str, kw=keyword: self._remove_tag(ts, kw)
                )
                chips.append(chip)
    ...
    def _load_and_display(self):
        ...
        if not self._dicom_files:
            self.header_output.value = (
                '<div style="color:#888;padding:20px;">No DICOM files available</div>'
            )
            return
        ...
        try:
            ...
            self.header_output.value = (
                f'<div style="color:red;padding:20px;">Error loading DICOM: {e}</div>'
            )
    ...
    def _format_element(self, elem, indent: int = 0) -> str:
        ...
        if elem.VR == "SQ":
            ...
            seq_html += (
                f'<span style="color:#888;">[SQ] {len(elem.value)} item(s)</span>'
            )
            seq_html += "</div>"
            ...
            for i, item in enumerate(elem.value):
                ...
                seq_html += (
                    f'<div style="color:#888;font-size:11px;">Item {i + 1}</div>'
                )
    ...
    def _update_display(self):
        ...
        if html_parts:
            self.header_output.value = (
                f'<div style="padding:10px;">{"".join(html_parts)}</div>'
            )
        else:
            self.header_output.value = (
                '<div style="color:#888;padding:20px;">No matching tags</div>'
            )
    ...
    def display(self):
        ...
        if self._show_title:
            header_items = [
                widgets.HTML(
                    f'<span style="font-weight:600;color:#1e293b;font-size:14px;">{self._title}</span>'
                ),
                widgets.HTML(
                    f'<span style="color:#94a3b8;font-size:11px;margin-left:10px;">{len(self._dicom_files)} file(s)</span>'
                ),
            ]
        ...
        quick_filter_row = widgets.HBox(
            [
                widgets.HTML(
                    '<span style="font-size:11px;color:#64748b;margin-right:5px;">Quick:</span>'
                ),
                self.quick_all_btn,
                ...
                widgets.HTML(
                    '<span style="font-size:11px;color:#64748b;margin-right:3px;">Groups:</span>'
                ),
            ]
